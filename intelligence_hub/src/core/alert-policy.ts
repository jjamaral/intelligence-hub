...
export function crossedMarketAlertThreshold(previousChangePercent:number|undefined,currentChangePercent:number,threshold:number):boolean{return Math.abs(currentChangePercent)>=threshold&&(previousChangePercent===undefined||Math.abs(previousChangePercent)<threshold);}
